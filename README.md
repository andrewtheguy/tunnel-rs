...
Keys are generated by the built-in `generate-auth-key` command. Key encoding,
file handling, and public-key derivation come from the shared
[`flexaccess-keys`](https://github.com/flexaccessdev/flexaccess-keys) crate, so
...
See the shared crate's
[key-format documentation](https://github.com/flexaccessdev/flexaccess-keys#key-format)
for the canonical encoding and authorized-keys grammar. To reprint the public
entry for a key you already have, use [`show-auth-key`](#show-auth-key).
