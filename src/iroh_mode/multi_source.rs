...
use crate::iroh_mode::endpoint::{
    RelayConfig, TUNNEL_ALPN, connect_to_server, create_client_endpoint,
    create_server_endpoint, validate_relay_only, watch_connection_paths,
};
use iroh::Endpoint;
...
pub async fn run_multi_source_server(config: MultiSourceServerConfig) -> Result<()> {
    ...
    accept_loop(&endpoint, &mut connection_tasks, &serve).await;
