...
1. **Shared key layer**: [`flexaccess-keys`](https://github.com/flexaccessdev/flexaccess-keys) owns the app-independent `ed25519-sec:` / `ed25519-pub:` format, secure key-file output, authorized-key parsing, and raw Ed25519 operations.
2. **Configuration**: `authorized_keys_file` points to shared-format public entries, and `private_key_file` points to the compact private-key file generated by `tunnel-rs generate-auth-key`.
