...
```bash
# Alice — generate a compact private key (uv-run script; key file to
# stdout, public entry to stderr)
scripts/generate-auth-key.py alice > alice.key 2> alice.pub
...
1. **Server Configuration**: `authorized_keys_file` points to public entries in `ed25519-pub:URLSAFE_BASE64[ comment]` form, where a single space delimits the optional comment.
2. **Client Configuration**: `private_key_file` points to the compact, prefixed private-key file generated by `scripts/generate-auth-key.py` (a uv-run Python script; the `ed25519-sec:`/`ed25519-pub:` prefixes name the algorithm, not tunnel-rs, so the keys are shareable across apps using the same scheme).
